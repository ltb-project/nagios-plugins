...
# 
# 2004 (c) Clement OUDOT <clement.oudot@linagora.com>
# 
...
if ( $warning_level =~ /^(\d*)$/ ) {
	...
	@warning_level = ( $1, $2 , $3 ) ;
	if ( $debug ) {
		print "Warning level set to $1 entries in transition, $2 rejected entries and $3 waiting entries\n" ;
	}
	$warning_level = 0 ;
}
else {
...
if ( $critical_level =~ /^(\d*)$/ ) {
	...
	if ( $debug ) {
		print "Critical level set to $1 entries in transition, $2 rejected entries and $3 waiting entries\n" ;
	}
	$critical_level = 0 ;
}
else {
...
sub usage {
	if ( $debug ) {
		print "Usage: $0 -w warning_level -c critical_level [-h hostname] [-p hostport] [-v]\n\n" ;
...
#==========================================================================
if ( $debug ) {
	print "Determining replication status for $slave_name at port $slave_port\n\n" ;
}
...
foreach ( $slapd_replog_file , $slurpd_replog_file , $status_file , $rej_file ) {
	...
	if ( ( -e $_ ) && ( ! -r $_ ) ) {
		...
		if ( $debug ) {
			print "File $_ exists but is not readable for the user running this script\n" ;
		}
		else {
...
if ( open ( STATUS , $status_file ) ) {
	...
	if ( defined $status_time ) {
		...
		if ( $debug ) {
			print "$slave_name (port $slave_port) is listed in $status_file\n\n" ;
		...
		if ( $debug ) {
			print "$slave_name (port $slave_port) is not listed in $status_file\n";
		...
		if ( ( $slave_is_concerned ) && ( /^time: (\d*)\.?(\d*)?$/ ) ) {
			...
			if ( $1 == $status_time ) {
				...
				if ( $replog_range > $status_range ) {
					$younger_entries++ ;
					if ( $debug ) {
						print "Entry with time $1 and range $replog_range is young\n" ;
					}
					next ;
				}
...
# Message to sent
print "$waiting_entries entries in transition, $rejected_entries entries rejected, $younger_entries entries waiting\n" ;

# exit code

if ( ( $critical_level && $max_errors >= $critical_level  ) || ( @critical_level && ( $critical_level[0] <= $waiting_entries || $critical_level[1] <= $rejected_entries || $critical_level[2] <= $younger_entries ) ) ) {
	exit $code{'Critical'} ;
}

if ( ( $warning_level && $max_errors >= $warning_level  ) || ( @warning_level && ( $warning_level[0] <= $waiting_entries || $warning_level[1] <= $rejected_entries || $warning_level[2] <= $younger_entries ) ) ) {
	exit $code{'Warning'} ;
}
