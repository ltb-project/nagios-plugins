...
"\tDefine if number returned should be greater or lesser thant thresold\n";
    ...
    print "\t\tversion: the directory's current version number\n";
    print "\t\tthreads: current number of active threads used\n";
    print "\t\tcurrentconnections: the number of connections currently in service\n";
    print "\t\ttotalconnections: the number of connections handled by the directory since it started\n";
    print "\t\tdtablesize: the number of file descriptors available to the directory\n";
    print "\t\treadwaiters: the number of threads waiting to read data from a client\n";
    print "\t\topsinitiated: the number of operations the server has initiated since it started\n";
...
if ( $type =~ /threads/i ) {
    ...
    $ldap_scope  ||= "base";
    $ldap_base   ||= "cn=Monitor";
    $ldap_attribute = "threads";
    $type_defined   = 1;
}
if ( $type =~ /currentconnections/i ) {
    $type_string = "currentconnections";
    $ldap_filter ||= "(objectClass=*)";
    $ldap_scope  ||= "base";
    $ldap_base   ||= "cn=Monitor";
    $ldap_attribute = "currentconnections";
    $type_defined   = 1;
}
if ( $type =~ /totalconnections/i ) {
    $type_string = "totalconnections";
    $ldap_filter ||= "(objectClass=*)";
    $ldap_scope  ||= "base";
    $ldap_base   ||= "cn=Monitor";
    $ldap_attribute = "totalconnections";
    $type_defined   = 1;
}
if ( $type =~ /dtablesize/i ) {
    $type_string = "dtablesize";
    $ldap_filter ||= "(objectClass=*)";
    $ldap_scope  ||= "base";
    $ldap_base   ||= "cn=Monitor";
    $ldap_attribute = "dtablesize";
    $type_defined   = 1;
}
if ( $type =~ /readwaiters/i ) {
    $type_string = "readwaiters";
    $ldap_filter ||= "(objectClass=*)";
    $ldap_scope  ||= "base";
    $ldap_base   ||= "cn=Monitor";
    $ldap_attribute = "readwaiters";
    $type_defined   = 1;
}
if ( $type =~ /opsinitiated/i ) {
    $type_string = "opsinitiated";
    $ldap_filter ||= "(objectClass=*)";
    $ldap_scope  ||= "base";
    $ldap_base   ||= "cn=Monitor";
    $ldap_attribute = "opsinitiated";
    $type_defined   = 1;
}
if ( $type =~ /opscompleted/i ) {
    $type_string = "opscompleted";
    $ldap_filter ||= "(objectClass=*)";
    $ldap_scope  ||= "base";
    $ldap_base   ||= "cn=Monitor";
    $ldap_attribute = "opscompleted";
    $type_defined   = 1;
}
if ( $type =~ /entriessent/i ) {
    $type_string = "entriessent";
    $ldap_filter ||= "(objectClass=*)";
    $ldap_scope  ||= "base";
    $ldap_base   ||= "cn=Monitor";
    $ldap_attribute = "entriessent";
    $type_defined   = 1;
}
if ( $type =~ /bytessent/i ) {
    $type_string = "bytessent";
    $ldap_filter ||= "(objectClass=*)";
    $ldap_scope  ||= "base";
    $ldap_base   ||= "cn=Monitor";
...
if ( $type =~ /starttime/i ) {
    $type_string = "starttime";
    $ldap_filter ||= "(objectClass=*)";
    $ldap_scope  ||= "base";
    $ldap_base   ||= "cn=Monitor";
    $ldap_attribute = "starttime";
    $type_defined   = 1;
}
if ( $type =~ /nbackends/i ) {
    $type_string = "nbackends";
    $ldap_filter ||= "(objectClass=*)";
    $ldap_scope  ||= "base";
    $ldap_base   ||= "cn=Monitor";
