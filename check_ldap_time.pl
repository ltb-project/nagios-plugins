...
sub test_ldap {
	...
	my $ldap = Net::LDAP->new( $host, port => $port, version => $version, timeout => $critical ) ;
	...
	if ( $binddn && $bindpw) {
		...
		if ( $req_bind->code ) {
			print "LDAP Unknown : Bind Error ".$req_bind->code." : ".$req_bind->error."\n";
			exit $code{Unknown} ;
		}
	}
	...
	my $req_search = $ldap->search ( base => '', scope => 'base' , filter => 'objectClass=*', attrs => ['1.1'] ) ;

	if ( $req_search->code ) {
		print "LDAP Unknown : Search Error ".$req_search->code." : ".$req_search->error."\n";
		$ldap->unbind ;
		exit $code{Unknown} ;
	}
