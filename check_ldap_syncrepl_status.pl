...
GetOptions(
	...
	'f'=> \$perf_data,'perf_data'=> \$perf_data,
	...
	#'o:s'=> \$oid,'oid:s'=> \$oid,
	# For LDAP plugins
	'D:s'=>\$ldap_binddn,'binddn:s'=>\$ldap_binddn,
	'P:s'=>\$ldap_bindpw,'bindpw:s'=>\$ldap_bindpw,
...
#=========================================================================
if ($version) {
	&print_revision($progname, "\$Revision: $VERSION (TPL: $TEMPLATE_VERSION)\$");
	exit $ERRORS{'UNKNOWN'};
}
...
sub get_ldapconn {
	...
	if ($useTls) {
		...
		$message->code && &verbose('1', $message->error) && return ($message->code, $message->error);
	...
	if ( $binddn && $bindpw) {
		...
		$req_bind->code && &verbose('1', $req_bind->error) && return($req_bind->code, $req_bind->error);
...
sub get_masteruri {
	...
	&verbose('2', "Found Master URI: ".$entry->get_value('monitorupdateref'));	
...
my $ldap_slave;
($errorcode, $ldap_slave) = &get_ldapconn($slave_uri, $ldap_binddn, $ldap_bindpw);
if ($errorcode) {
...
if (!$ldap_suffix) {
	...
	if ($errorcode) {
		print "Can't get suffix from $slave_uri. Please provide it with -S option.\n";
		exit $ERRORS{'UNKNOWN'};
	}
}
...
if (!$master_uri) {
	...
	if ($errorcode) {
		print "Can't get Master URI from $slave_uri. Please provide it with -U option.\n";
		exit $ERRORS{'UNKNOWN'};
	}
}
...
my $ldap_master;
($errorcode, $ldap_master) = &get_ldapconn($master_uri, $ldap_binddn, $ldap_bindpw);
if ($errorcode) {
...
($errorcode, $slavecsn) = &get_contextcsn($ldap_slave, $ldap_suffix, $ldap_serverid);
if ($errorcode) {
	print "Can't get Context CSN with SID $ldap_serverid from $slave_uri. Please set SID with -I option.\n";
	exit $ERRORS{'UNKNOWN'};
}

($errorcode,$mastercsn) = &get_contextcsn($ldap_master, $ldap_suffix, $ldap_serverid);
if ($errorcode) {
	print "Can't get Context CSN with SID $ldap_serverid from $master_uri. Please set SID with -I option.\n";
	exit $ERRORS{'UNKNOWN'};
}
...
if ($deltacsn == 0) {
	...
	if ($deltacsn < $warning) {
		print "OK - directories are not in sync - $deltacsn seconds late (W:$warning - C:$critical)$perfparse";	
		exit $ERRORS{'OK'};
	} elsif ($deltacsn > $warning and $deltacsn < $critical) {
		print "WARNING - directories are not in sync - $deltacsn seconds late (W:$warning - C:$critical)$perfparse";	
		exit $ERRORS{'WARNING'};
	} else {
		print "CRITICAL - directories are not in sync - $deltacsn seconds late (W:$warning - C:$critical)$perfparse";	
		exit $ERRORS{'CRITICAL'};
	}
}
